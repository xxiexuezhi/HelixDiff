...
import time
from utils import random_mask_batch, get_condition_from_batch, selected_mask_batch,random_mask_given_all_except_3hotspots_batch
import shutil
...
def main():
    ...
    parser.add_argument('--resume', type=str, default="/home/xxie92/scratch/helisgm_1_64_64_len14_fixed_training")
   ...
   # dataset = ProteinDataset(config.data.dataset_path, config.data.min_res_num,
    ...
    for step in range(initial_step, config.training.n_iters + 1):
        ...
        #batch = random_mask_given_all_except_3hotspots_batch(batch, config)
        ...
        if step % config.training.eval_freq == 0:
            ...
            # the below generted mask matrix only given 3 hotspot seq info. model need to generate all the rest
            #eval_batch = random_mask_given_all_except_3hotspots_batch(eval_batch, config)
