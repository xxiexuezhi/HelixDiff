...
target_coords_NC = np.array([[151.363, 143.555, 118.29 ],
       [153.025, 143.292, 121.633],
       [152.483, 142.65 , 122.907],
       [152.787, 141.177, 122.975],
       [156.681, 144.902, 118.495],
       [156.483, 146.652, 116.696],
       [157.724, 146.565, 116.597],
       [160.764, 149.036, 121.4  ],
       [158.522, 150.624, 117.172],
       [158.201, 150.855, 115.854],
       [169.515, 149.553, 125.41 ]], dtype="float32")

# change L type hotspots to D type
target_coords_NC = target_coords_NC * -1
#####target_atom_name = target_atom_names[kkk]
target_atom_name =['CA_THR_13',
 'CB_THR_13',
 'CG2_THR_13',
 'CA_ASP_15',
 'CG_ASP_15',
...
#    assert not (args.pdb is not None and args.select_length)
    ...
    workdir = Path("sampling", "case_study_glp1_helix2", Path(args.config).stem, Path(args.checkpoint).stem, args.tag)
