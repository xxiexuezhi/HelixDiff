...
#    assert not (args.pdb is not None and args.select_length)
    ...
    workdir = Path("sampling", "coords_6d","single_test_index0_hot159_fixed", Path(args.config).stem, Path(args.checkpoint).stem, args.tag)
    ...
    for _ in tqdm(range(args.n_iter)):
        ...
        # I conmmented it out for simple purpse. I will change to my data later.
            ...
            condition = get_conditions_by_specify_hotspots(args.pkl, index, config, args.chain,args.batch_size, '5,10,11', 'Y,A,V')            
